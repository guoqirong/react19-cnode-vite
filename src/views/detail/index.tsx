...
const Detail: FunctionComponent = () => {
  ...
  const topicListParm = searchParams.get('listParm');
  ...
  const getData = () => {
    startTransition(
      async () =>
        await httpRequest({
          url: adornUrl(`/api/v1/topic/${topicId}`),
          method: 'get',
          params: {
            mdrender: true,
  ...
  const goback = () => {
    if (topicListParm) {
      dispatch(updateListParm(topicListParm));
      history('/');
    } else if (userName) {
  ...
  const editTopic = () => {
    if (userName) {
      history(`/edit-topic/${topicId}?userName=${userName ?? ''}`);
    } else {
      history(`/edit-topic/${topicId}?listParm=${topicListParm ?? ''}`);
    }
  };
  ...
  const replieTopicRequest = (values: { content: string; reply_id: string }) => {
    httpRequest({
              ...
              <PageHeader
                ...
                operation={
                  simpleUserData.loginName === topic?.author.loginname && (
                    <Button className='site-page-header-btn' type='primary' onClick={editTopic}>
                      编辑话题
                    </Button>
                  )
                }
        ...
        >
          <div className='detail-topic-title'>
            <div className='title-left'>
              ...
              >
                <Tag color={topic?.top ? 'red' : 'green'}>{getTopicTab(topic?.top, topic?.tab)}</Tag>
              </div>
              <div
              ...
              <div className='topic-title-desc'>
                {'● ' +
                  ...
                  ' ● ' +
                  (topic?.author && topic?.author.loginname ? topic?.author.loginname : '')}
              </div>
            </div>
              ...
              return (
                <div className='replie-item' key={i}>
                  <div className='replie-header'>
                    ...
                    <div className='replie-up' onClick={() => likeAndUnlike(item.id)}>
                      <div className='icon up-icon'></div>
                      {!!item.ups.length && <span>{item.ups.length}</span>}
                    </div>
                    <div className='icon replie-icon' onClick={() => changeRepliceItemState(i)}></div>
                  </div>
                  <div className='replie-content' dangerouslySetInnerHTML={{ __html: item.content }}></div>
                  {item.isReplie && (
                    <Form
                      ref={(el) => {
                        replieForms['replieForm' + i] = el as FormInstance<any>;
                        return () => {
                          replieForms['replieForm' + i] = null;
                        };
                      }}
                    ...
                    >
                      <Form.Item
                        ...
                        label=''
                        getValueFromEvent={() => {
                          return replieForms['replieForm' + i]?.getFieldValue('replieContent' + i);
                        }}
                        getValueProps={(value) => {
                          return value;
                        }}
                      ...
                      >
                        <Editor
                          ...
                          value={replieForms['replieForm' + i]?.getFieldValue('replieContent' + i)}
                          onEditorChange={(v) => {
