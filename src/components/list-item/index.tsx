...
const ListItem: FunctionComponent<ListItemProps> = ({ topicItem, isSimpleItem, onItemClick }) => {
  return (
    <List.Item
      key={topicItem.title}
      className='item-body'
          ...
          <div>
            ...
            {!isSimpleItem && (
              <div
                ...
                style={{
                  width:
                    !(topicItem as topicListItemType).top && (topicItem as topicListItemType).tab === 'dev'
                      ? ''
                      : '50px',
                }}
              >
                <Tag color={(topicItem as topicListItemType).top ? 'red' : 'green'}>
                  {getTopicTab((topicItem as topicListItemType).top, (topicItem as topicListItemType).tab)}
                </Tag>
              </div>
            )}
            <div
            ...
            {!isSimpleItem && (
              <div className='created-time'>{formatDate((topicItem as topicListItemType).create_at, 'yyyy-MM-dd')}</div>
            )}
          </div>
        }
      ></List.Item.Meta>
